...
import { ensureCoursesSchema, ensurePrefsSchema, logEvent, storeCourseText } from '../_lib/db.js';
...
// html/md：≤1.4MB 存 D1，超过自动转存 R2，故整体上限放到 25MB（大网页/自包含 HTML 也能传）
const MAX_TEXT_BYTES = 25_000_000;
...
export async function onRequestPost({ request, env }) {
  ...
  // html/md：小文本进 D1、大文本转存 R2（htmlColumn 为空即「正文在 R2」标记）
  let htmlColumn = '';
  if (kind !== 'pdf') {
    try {
      htmlColumn = await storeCourseText(env, file, contentText);
    } catch (e) {
      return err('正文存储失败（大文件需 R2，未配置？）', 500);
    }
  }
  ...
  await env.DB.prepare(
    `INSERT INTO courses (file, title, subject, description, icon, color, tags, html, kind, category, status, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(file, title.slice(0, 80), finalSubject, finalDesc, finalIcon, finalColor, finalTags, htmlColumn, kind, finalCategory, status, Date.now()).run();
...
export async function onRequestPut({ request, env }) {
  ...
  if (typeof body.content === 'string') {
    ...
    // 大文本转存 R2、小文本回 D1（storeCourseText 内部会清理另一边的旧副本）
    const htmlColumn = await storeCourseText(env, file, body.content);
    sets.push('html = ?'); binds.push(htmlColumn);
  }
...
export async function onRequestDelete({ request, env }) {
  ...
  if (file.startsWith('u-')) {
    // 动态课程：内容在 D1/R2，物理删除。R2 对象按 file slug 删（pdf 及大 html/md 都用此 key），尽力而为
